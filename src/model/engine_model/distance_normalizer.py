...
class DistanceNormalizer:
    ...
    def normalize(self, distance_costs_trucks, occupation_costs_trucks, time_costs_trucks):
        if self.is_verified:
            ...
            self.normalized_trucks_distance = np.array([[dist[0]] for dist in distance_costs_trucks])
            self.normalized_trucks_occupation = np.array([[occupation[0]] for occupation in occupation_costs_trucks])
            self.normalized_trucks_time = np.array([[time[0]] for time in time_costs_trucks])
